...
def _process_inputs(points, fp16):
    voxels, coords, num_points = voxelization(points, voxel_generator) #voxel_generator.generate(points)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="RVCDet")
    parser.add_argument("config", default="./configs/waymo/pp/waymo_rvcdet_pp_two_pfn_stride1_3x.py", help="path to config file")
    parser.add_argument(
        "--checkpoint", help="the path to checkpoint which the model read from", default="./weights/pointpillars_waymo.pth", type=str
    )
    parser.add_argument('--input_data_dir', default="/media/hdd/benchmarks/Waymo/val_test/lidar", type=str, required=True)
    ...
    for frame_name in tqdm(sorted(os.listdir(args.input_data_dir))):
        ...
        points = np.hstack((points,np.zeros((points.shape[0], 2))))                      # Intensity, Ring
        points = np.hstack((points,np.ones((points.shape[0], 1))))                       # Radar
        points = np.hstack((points, 3 * np.ones((points.shape[0], 1))))                       # Velocity
