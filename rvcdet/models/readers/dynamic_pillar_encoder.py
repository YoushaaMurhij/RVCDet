...
class DynamicPillarFeatureNet(nn.Module):
    ...
    def forward(self, points):
        ...
        dtype = points.dtype
        keep =  (points[:, 0] >= self.pc_range[0]) * (points[:, 0] <= self.pc_range[3]) * \
        ...
        # id = torch.ones((points.shape[0],1)).type_as(points).to(device)
        # points = torch.hstack((id,points)).type_as(points).to(device)
        # points = F.pad(input=points, pad=(1, 0, 0, 0), mode='constant', value=1)

        # attention: points in format (batch_id, x,y,z,...)
        points_coords = (points[:, 1:3] - pc_range[:2].view(-1, 2))/ voxel_size[:2].view(-1, 2)   #x,y
    ...
    def voxelization(self, points, pc_range, voxel_size):    
        ...
        voxels = torch_scatter.scatter_mean(points, inverse_indices, dim=0)
