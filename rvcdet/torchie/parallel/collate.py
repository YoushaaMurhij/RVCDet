...
def collate_kitti(batch_list, samples_per_gpu=1):
    ...
    for index, example in enumerate(batch_list):
        if type(example) is list:
            ...
            for k, v in example.items():
                if k == 'points':
                    v= np.pad(v, (1,0), 'constant', constant_values=(index,))
                example_merged[k].append(v)
    ...
    for key, elems in example_merged.items():
        if key in ["points", "voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
